...
/**
 * Test: laps.list senza filtri.
 * Atteso: 8 lap totali, ordinati per tempo crescente.
 * Il primo deve essere LAP006 (108200ms = 1:48.200, Monza Porsche).
 */
function testLapsList() {
  const login = handleAuthLogin({ code: 'CRUCITTI-9182' });
  const ctx = verifyToken(login.data.token);

  const result = handleLapsList({}, ctx);
  Logger.log('laps.list result:');
  Logger.log(`Totale: ${result.data.count}`);
  Logger.log('Top 3 più veloci:');
  result.data.laps.slice(0, 3).forEach(l => {
    Logger.log(`  ${l.lap_id} | ${l.driver_id} | ${l.lap_time_display} | ${l.track_id}`);
  });
}

/**
 * Test: leaderboard per Spa LMU + Ferrari 296 GT3.
 * Atteso: 2 piloti (VSD005 con 136200, VSD007 con 138100).
 * VSD005 ha 2 lap (LAP001 137500, LAP002 136200) → leaderboard prende LAP002.
 */
function testLapsLeaderboardSpaFerrari() {
  const login = handleAuthLogin({ code: 'CRUCITTI-9182' });
  const ctx = verifyToken(login.data.token);

  const result = handleLapsLeaderboard({
    sim: 'LMU',
    track_id: 'lmu-spa-gp',
    car_id: 'lmu-ferrari-296-gt3',
  }, ctx);

  Logger.log('leaderboard Spa LMU Ferrari result:');
  Logger.log(`Piloti in classifica: ${result.data.count}`);
  result.data.laps.forEach((l, i) => {
    Logger.log(`  ${i + 1}. ${l.driver_id} - ${l.lap_time_display} (${l.lap_id})`);
  });

  // Verifica logica
  if (result.data.count === 2 &&
      result.data.laps[0].driver_id === 'VSD005' &&
      result.data.laps[0].lap_id === 'LAP002') {
    Logger.log('✓ Leaderboard: VSD005 (LAP002, best) > VSD007 (LAP003)');
  } else {
    Logger.log('⚠️ Leaderboard logica errata');
  }
}

/**
 * Test: leaderboard senza car_id (tutte le auto su Spa LMU).
 * Atteso: 4 piloti (VSD002, VSD005, VSD007, VSD013).
 */
function testLapsLeaderboardSpaAllCars() {
  const login = handleAuthLogin({ code: 'CRUCITTI-9182' });
  const ctx = verifyToken(login.data.token);
  ...
  Logger.log('leaderboard Spa LMU all cars result:');
  Logger.log(`Piloti: ${result.data.count}`);
  result.data.laps.forEach((l, i) => {
    Logger.log(`  ${i + 1}. ${l.driver_id} - ${l.lap_time_display} (${l.car_id})`);
  });

  if (result.data.count === 4) {
    Logger.log('✓ Tutti e 4 i piloti di Spa LMU presenti');
  } else {
    Logger.log(`⚠️ Atteso 4 piloti, trovati ${result.data.count}`);
  }
}
